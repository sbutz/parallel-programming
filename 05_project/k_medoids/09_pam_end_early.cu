...
__global__ void swap_cost(const value_type *points, const size_type n_points,
    ...
    extern __shared__ value_type local_medoids[];
    if (threadIdx.y < n_clusters) {
        local_medoids[threadIdx.y * D_POINTS] = points[medoids[y] * D_POINTS];
        local_medoids[threadIdx.y * D_POINTS + 1] = points[medoids[y] * D_POINTS + 1];
    }
    __syncthreads();
    ...
    if (!is_medoid) {
        ...
        for (size_type i = 0; i < n_points; ++i) {
            value_type min_distance =
                euclidean_distance(&points[i * D_POINTS], &points[x * D_POINTS]);
            for (size_type j = 0; j < n_clusters; j++) {
                if (j != y) {
                    const value_type d =
                        euclidean_distance(&points[i * D_POINTS], &local_medoids[j * D_POINTS]);
                    if (d < min_distance) {
                        min_distance = d;
                    }
                }
...
std::vector<size_type> pam(const Args<size_type, value_type> &args) {
    ...
    dim3 blockSize(next_smaller_power_of_two(N_THREADS / args.n_clusters), args.n_clusters, 1);
    assert(blockSize.x * blockSize.y <= N_THREADS);
    assert(blockSize.y >= args.n_clusters);
    dim3 gridSize((args.n_points + blockSize.x - 1) / blockSize.x,
                  (args.n_clusters + blockSize.y - 1) / blockSize.y, 1);
    const size_type shmem_size = args.n_clusters * args.d_points * sizeof(value_type);
    ...
    for (i = 0; i < MAX_ITERATIONS; i++) {
        ...
        swap_cost<<<gridSize, blockSize, shmem_size>>>(d_points, args.n_points, args.n_clusters,
                                                       d_medoids, d_swaps);
