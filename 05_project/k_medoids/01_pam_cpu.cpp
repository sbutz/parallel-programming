...
double total_cost(const double *points, std::size_t n, const double *medoids, std::size_t k,
                  std::size_t size) {
    double cost = 0.0;
    for (std::size_t i = 0; i < n; i += 1) {
        ...
        for (std::size_t j = 0; j < k; j += 1) {
            min_distance = std::min(
                min_distance, euclidean_distance(&points[i * size], &medoids[j * size], size));
        }
        cost += min_distance;
    }
    return cost;
...
double *pam(const double *points, std::size_t n, std::size_t size, std::size_t k) {
    ...
    do {
        ...
        // perform best swap
        if (best_cost < cost) {
            memcpy(&medoids[best_swap.first * size], &points[best_swap.second * size],
                   size * sizeof(double));
            cost = best_cost;
        } else {
            break;
...
void pam_wrapper(const Args<std::size_t, double> &args) {
    double *medoids = pam(args.data.data(), args.n_points, args.d_points, args.n_clusters);
    // print_values(medoids, args.n_clusters, args.d_points);
    free(medoids);
}
