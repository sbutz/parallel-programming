...
static constexpr std::uint32_t D_POINTS = 2;
...
__device__ double euclidean_distance(const double *a, const double *b) {
    return pow(a[0] - b[0], 2) + pow(a[1] - b[1], 2);
}

__global__ void swap_cost(const double *points, const std::size_t n_points,
                          const std::size_t n_clusters, const std::size_t *medoids, swap *swaps) {
