...
__global__ void dgemm_gpu_simple(const float* a, const float* b, float* c, const int n) {
	...
	for(int i=0;i<n;++i)
		c[id] += a[vx+i] * b[vy+i*n];
}

__global__ void dgemm_gpu_SharedMemory(const float* a, const float* b, float* c, const int n) {
...
int main(int argc, const char** argv) {
	...
	//for (int i = 0; i < n * n; ++i)testproc(h_a, h_b, h_c, n, i);
