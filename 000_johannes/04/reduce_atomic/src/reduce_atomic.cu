...
__global__ void deviceReduceKernelAtomic(float *in, float* out, size_t N) {
	...
	if (tid == 0) atomicAdd(out, s_sums[0]);
}
...
float reduction(float *in, size_t N)
{
	...
	deviceReduceKernelAtomic<<<blocks, threads, threads * sizeof(float)>>> (in, out, N);
