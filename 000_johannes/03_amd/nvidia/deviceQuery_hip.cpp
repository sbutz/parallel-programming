// Translated from HIP to HIP with hipify-perl, manual adjustments
 ...
 *
 */
/* This sample queries the properties of the HIP devices present in the system
 * via HIP Runtime API. */
...
// This function wraps the HIP Driver API into a template function
template <class T>
inline void getHIPAttribute(T *attribute, hipDeviceAttribute_t device_attribute,
                             int device) {
...
int main(int argc, char **argv) {
  ...
  if (deviceCount == 0) {
    ...
    printf("Detected %d HIP Capable device(s)\n", deviceCount);
  }
  ...
  for (dev = 0; dev < deviceCount; ++dev) {
    ...
    printf("  (%2d) Multiprocessors, (%3d) HIP Cores/MP:     %d HIP Cores\n",
...
#else
    // This only available in HIP 4.0-4.2 (but these were only exposed in the
    // HIP Driver API)
    int memoryClock;
    getHIPAttribute<int>(&memoryClock, hipDeviceAttributeMemoryClockRate,
                          dev);
    printf("  Memory Clock rate:                             %.0f Mhz\n",
           memoryClock * 1e-3f);
    int memBusWidth;
    getHIPAttribute<int>(&memBusWidth,
                          hipDeviceAttributeMemoryBusWidth, dev);
    printf("  Memory Bus Width:                              %d-bit\n",
           memBusWidth);
    int L2CacheSize;
    getHIPAttribute<int>(&L2CacheSize, hipDeviceAttributeL2CacheSize, dev);
...
//           deviceProp.computePreemptionSupported ? "Yes" : "No");
  ...
  // csv masterlog info
  // *****************************
  // exe and HIP driver name
  printf("\n");
  std::string sProfileString = "deviceQuery, HIP Driver = HIPRT";
  char cTemp[16];

  // driver version
  sProfileString += ", HIP Driver Version = ";
#if defined(WIN32) || defined(_WIN32) || defined(WIN64) || defined(_WIN64)
  ...
  // Runtime version
  sProfileString += ", HIP Runtime Version = ";
#if defined(WIN32) || defined(_WIN32) || defined(WIN64) || defined(_WIN64)
