...
void get_image (const char* name, float** f_buffer, int *w, int *h, int *channel ) {
	int rc, i;
...
//		printf( "Input: Read %d/%lu bytes", rc, jpg_size-i);
		i += rc;
	}
	close(fd);
...
//   SSS       T     A     A  R     R     T   
	...
	*channel = cinfo.output_components;
	...
	bmp_size = (*w) * (*h) * (*channel);
	buffer =(unsigned char*) malloc(bmp_size*sizeof(unsigned char));

	// The row_stride is the total number of bytes it takes to store an
	// entire scanline (row). 
	row_stride = (*w) * (*channel);
    ...
    *f_buffer =(float*) malloc(sizeof(float)*bmp_size);
    for(int j=0; j<bmp_size; j++) 
        (*f_buffer)[j] = (float) buffer[j];
