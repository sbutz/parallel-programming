...
__global__ void mmult_coalesce_kernel(
	float * C, float * A, float * B, size_t m, size_t k, size_t n
) {
	size_t r = blockIdx.x * blocksize + (threadIdx.x / blocksize);
  size_t c = blockIdx.y * blocksize + (threadIdx.x % blocksize);
...
int main () {
  ...
  mmult_coalesce_kernel<<<dimGrid, dimBlock>>> (deviceC, deviceA, deviceB, M, K, N);
