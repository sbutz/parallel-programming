...
char const * usageText = "Usage: c_boehm input_file n r [w] [iterations]\n";
...
static auto runDbscan (
  ...
  for (auto && step : steps) {
    auto && clst = std::get<0> (step);
    auto && isc = std::get<1> (step);
    for (std::size_t i = 0; i < clst.size(); ++i) if (isc[i] || clst [i]) clst[i] += i + 1;
  }
  struct Result {
  ...
  return Result { std::move(isCore), std::move(clusters), std::move(steps) };
}
  ...
  std::cout << "{\n";
    ...
    std::cout << "\"steps\": [\n";
      ...
      for (auto && step : steps) {
        if (!first) { std::cout << ",\n"; } else { first = false; }
        std::cout << "{\n";
        std::cout << "\"endCur\": " << std::get<2>(step) << ",\n";
        std::cout << "\"is_core\": "; jsonPrintUnsignedIntegerVector(std::get<1>(step)); std::cout << ",\n";
        std::cout << "\"clusters\": "; jsonPrintUnsignedIntegerVector(std::get<0>(step)); std::cout << "\n";
        std::cout << "}";  
      }
    std::cout << "\n],\n";
    std::cout << "\"profiles\": [\n";
