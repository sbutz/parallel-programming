...
int main() {
  ...
  hipGetDeviceProperties(&deviceProp, 3);
  ...
  std::cerr << numBlocksPerSm << "\n";
    ...
    if (!deviceProp.cooperativeLaunch) {
        std::cerr << "Cooperative kernel launch is not supported on this device." << std::endl;
        return -1;
    }
  ...
  float * d_result;
  CUDA_CHECK(hipMalloc(&d_result, nSampleData * sizeof(float)));
  ...
  /*hipLaunchCooperativeKernel(
    ...
    dim3{2}, dim3{cWarpSize * cWarpsPerBlock},
    kernelArgs, nSharedFloats * sizeof(float), 0
  );*/


  void * aaa [] = {};
  hipLaunchCooperativeKernel((void *)dummy, dim3{2}, dim3{cWarpSize * cWarpsPerBlock},
    aaa, 0, 0);
  CUDA_CHECK(hipGetLastError());

  float w;
//  CUDA_CHECK(hipMemcpy(&w, &(d_result.data()[6208]), sizeof(float), hipMemcpyDeviceToHost));

//  std::cerr << nSampleData << " " << w << "\n";

  CUDA_CHECK(hipMemcpy(&result, d_result, nSampleData * sizeof(float), hipMemcpyDeviceToHost))
