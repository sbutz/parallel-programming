...
#define NUM_POINTS 1024 * 8 // Maximum NUM_POINTS * NUM_K : 1024 * 64 
#define NUM_K 8
...
#pragma unroll	
	...
	while (changed && !maxlooped)
	{
		...
		reduction_arr_modulo << <blocks_arr, NUM_THREADS >> > (dcxsum_arr, dcxsum);
		reduction_arr_modulo << <blocks_arr, NUM_THREADS >> > (dcysum_arr, dcysum);
		reduction_arr_modulo << <blocks_arr, NUM_THREADS >> > (dctotal_arr, dctotal);
		cudaDeviceSynchronize();
