...
__global__ void assign_Points(Point* p, Centroid* c) {
    ...
    for (unsigned int j = 0; j < NUM_K; ++j) {
        distance_dev(pdistx, pdisty, scX[j], scY[j], dist);
    ...
    atomicAdd(&c[cluster].xsum, pdistx);
    atomicAdd(&c[cluster].ysum, pdisty);    
}
...
__global__ void assign_PointsTest(Point* p, Centroid* c,int* testwert) {
    ...
    //3. Reduktion Kaskade und Shuffle
...
int main()
{
    ...
    while (changed) {
        ...
        //resete Cluster-Eigenschaften - wird hier auskommentiert, da in Kernel verlegt
        /*for (int i = 0; i < NUM_K; ++i) {
            hc[i].xsum = 0;
            hc[i].ysum = 0;
            hc[i].total = 0;
