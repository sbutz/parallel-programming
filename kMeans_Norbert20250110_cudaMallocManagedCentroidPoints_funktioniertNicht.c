...
int main()
{
	...
	//Speicherallokation
	Point* p;  cudaStatus = cudaMallocManaged(&p, csize); if (cudaFail(cudaStatus, "Fehler bei cudaMallocMamaged für p"))return -1;	

	// Centroids
	Centroid* c;  cudaStatus = cudaMallocManaged(&c, csize); if (cudaFail(cudaStatus, "Fehler bei cudaMallocMamaged für c"))return -1;
	Centroid* oc = (Centroid*)malloc(csize);             // alte Zentroiddaten zum Abgleich einer Änderung
		
	//Initialisierung der Datenpunkte		
	if (READ_FROM_FILE) {
		int anz = getPoints(p, POINTSFILE, 65);
		if (-1 == anz) {
		...
		for (unsigned long i = 0; i < NUM_POINTS; ++i) {
			p[i].x = rand() % MAX_X; // NotlÃ¶sung
			p[i].y = rand() % MAX_Y; // NotlÃ¶sung
		}
	}

	if (SAVE_TO_FILE)savePoints(p);
	...
	while (changed && !maxlooped)
	{
		...
		if (SHARED) {
			if ((NUM_K <= NUM_THREADS)&&SHARED_EXT)				
				kMeansClusterAssignSharedMemoryExt << <blocks, NUM_THREADS >> > (p, c);
			else
				kMeansClusterAssignSharedMemory << <blocks, NUM_THREADS >> > (p, c);
		}
		else
			kMeansClusterAssign << <blocks, NUM_THREADS >> > (p, c);
		...
		if(SHARED)
			kMeansCentroidUpdateSharedMemory << <blocks, NUM_THREADS >> > (p, c, true);
		else
			kMeansCentroidUpdate << <blocks, NUM_THREADS >> > (p, c, true);
		cudaDeviceSynchronize();//*/
		...
		for (int i = 0; i < NUM_K; ++i) {
			...
			if ((c[i].x != oc[i].x) || (c[i].y != oc[i].y))changed = true;
