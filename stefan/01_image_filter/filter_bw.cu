...
int main(int argc, char* argv[])
{
    ...
    GpuAssert(cudaMalloc((void**)&dInputImage, width * height * 3));
    GpuAssert(cudaMalloc((void**)&dOutputImage, width * height));

    GpuAssert(cudaMemcpy(dInputImage, hInputImage.GetRawData(), width * height * 3, cudaMemcpyHostToDevice));
    ...
    Jpeg hOutputImage{width, height};
